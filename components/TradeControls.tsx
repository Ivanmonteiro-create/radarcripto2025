...
export default function TradeControls({ symbol, onSymbolChange, livePrice, onBuy, onSell }: Props) {
  ...
  function execute(side: TradeSide) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Operação simulada recusada");
      setSnapshot(engine.snapshot());
    }
  }
  ...
  return (
    <div className="tcRoot">
      <div className="tcHead"><h3>Controles de Trade · Spot SIM</h3><small className="muted">Taxa 0,10% · Slippage 1 bps</small></div>
      <div className="tcGrid">
        <div className="col">
          <label className="lbl">Par</label><select className="inp" value={symbol} onChange={(event) => changeSymbol(event.target.value as Pair)}>{PAIRS.map((pair) => <option key={pair}>{pair}</option>)}</select>
          <label className="lbl">Preço ao vivo</label><input className="inp" disabled value={livePrice ? fmt(livePrice, 8) : "—"} />
          <label className="lbl">Limite de risco por operação (%)</label><input className="inp" type="number" min={0.1} max={100} step={0.1} value={riskPct} onChange={(event) => setRiskPct(Number(event.target.value))} />
          <label className="lbl">Tamanho solicitado (USDT)</label><input className="inp" type="number" min={0} value={sizeUSDT} onChange={(event) => setSizeUSDT(Number(event.target.value))} />
          <small className="muted">Executável após risco: {fmt(executableUSDT)} USDT</small>
        </div>
        <div className="col">
          <Metric label="Saldo livre" value={fmt(snapshot.cash)} />
          <Metric label="Equity" value={fmt(snapshot.equity)} />
          <Metric label="PNL realizado" value={fmt(snapshot.realizedPnl)} />
          <Metric label="PNL não realizado" value={fmt(snapshot.unrealizedPnl)} />
          <Metric label="Posição" value={position ? `${position.quantity.toFixed(8)} @ ${fmt(position.averageEntryPrice, 8)}` : "—"} />
        </div>
        ...
        <div className="row twoCols"><button className="btn btnBuy" disabled={!livePrice || executableUSDT <= 0} onClick={() => execute("BUY")}>Comprar</button><button className="btn btnSell" disabled={!livePrice || !position} onClick={() => execute("SELL")}>Vender</button></div>
        <div className="row twoCols"><button className="btn" onClick={reset}>Resetar simulação</button><button className="btn" onClick={exportCsv}>Exportar CSV</button></div>
        {error && <div className="row pnlNeg">{error}</div>}
        <div className="row histCard"><strong>Histórico</strong>{snapshot.trades.slice().reverse().slice(0, 15).map((trade) => <div key={trade.id} className="histLine"><span>{new Date(trade.timestamp).toLocaleString()}</span><span>{trade.side}</span><span>{trade.symbol}</span><span>{fmt(trade.price, 8)}</span><span>fee {fmt(trade.feeQuote, 4)}</span><span>PNL {fmt(trade.realizedPnl)}</span></div>)}</div>
      </div>
      <style jsx>{`
